# Bridge and Baseline — YouTube Content Research
**Date:** June 2026

---

## Channel Context (from existing docs)

- **Niche:** Guitar setup, ergonomics, and adaptive playing for adult guitarists aged 35–65
- **Audience:** Players fighting friction — arthritis, reduced grip, returning players, post-injury hands
- **Core format:** One friction point → PM insight → Fix Ladder ($0 / $6–20 / $25–100 / $100+) → before/after demo → 30 sec uninterrupted playing
- **Mission:** "Luxury is the absence of friction. Easier is not a shortcut — easier is the point."
- **Already scripted (Videos 1–9):** Setup basics, arthritis (PTI Ep 1), pawn shop rescue, barre chords, $4k mistake, light strings, ergonomics, identity video, PTI series intro

---

## Trending Topics & Keywords

**High search volume, well-matched to channel format:**
- "guitar action too high" — extremely high search, clear DIY angle
- "guitar buzzing / fret buzz fix" — one of the highest-volume guitar queries
- "guitar setup cost" — emotional search; audience wants to know if they're overpaying
- "how to play guitar with arthritis" — growing search, underserved in quality content
- "guitar for beginners over 50" — rising demographic
- "silent guitar practice apartment" — urban lifestyle trend, steady growth
- "short scale guitar benefits" — growing interest, especially among older players
- "when to change guitar strings" — perennial beginner entry point
- "what is neck relief" — confusing term, high frustration signal
- "how to intonate a guitar" — high search, almost no beginner-accessible content

**Emerging / underserved angles (2025–2026):**
- Carpal tunnel + guitar (distinct from arthritis — compression vs. inflammation; no one covers this clearly)
- Guitar humidity and seasonal setup drift (recurring need, no strong beginner resource exists)
- Ergonomic guitars and body weight (Strandberg-type coverage mostly ignores the 45-year-old casual player)
- Spatial audio / headphone amp comparison (Boss Waza Air, Fender Mustang Micro trending in silent practice)

---

## New Video Ideas by Series

### Playing Through It (Series expansion beyond arthritis)

| # | Working Title | Key Angle |
|---|---|---|
| Ep 2 | **Carpal Tunnel and Guitar: What Actually Helps** | Different mechanism from arthritis; setup fixes the variable cost, not the fixed cost |
| Ep 3 | **Tendinitis: Playing Without Making It Worse** | Active management; warm-up protocol; string gauge as first fix |
| Ep 4 | **Trigger Finger and the Guitar** | Almost no YouTube content exists; large search signal in forums |
| Ep 5 | **One-Handed Playing: The Django Framework** | Django already referenced in docs; expand into techniques, partial chord shapes |
| Ep 6 | **Neuropathy: When Fingers Go Numb** | Guitar forums full of this question; direct, clinical tone |
| Ep 7 | **Shoulder and Neck Pain from Playing** | Posture + strap height = setup problem disguised as technique problem |
| Ep 8 | **Playing Through a Flare-Up** | The episode for when adaptation isn't enough — what to do that day |

---

### Guitar Setup Basics (High-volume search topics)

- **"Guitar Buzzing? Here's the $0 Fix Most Players Miss"** — nut slots; highest-leverage 15-min check
- **"What Is Neck Relief? (And Why Your Guitar Has Too Much)"** — explainer; demystifies a scary-sounding term
- **"Guitar Action Too High: Fix It Yourself Before You Pay Anyone"** — counter-programmes luthier searches
- **"How to Intonate a Guitar (Beginner Version)"** — almost no clear beginner content exists
- **"Guitar Setup: Pay a Luthier or DIY?"** — honest cost breakdown; positions the Fix Ladder perfectly
- **"Why Your Guitar Goes Out of Tune"** — nut binding, string age, tuner slippage; covers the top 3 causes
- **"Acoustic vs. Electric Action: Different Standards, Different Rules"** — common confusion, clear explainer
- **"Humidity and Your Guitar: Seasonal Setup Drift Explained"** — recurring seasonal content, easy to reshoot

---

### Silent & Stealth Practice (Strong trending niche)

- **"The £0 Silent Practice Trick"** — foam under strings; before/after sound demo
- **"The Complete Silent Practice Rig: Fix Ladder £0 → £15 → £150"** — anchor video for the series
- **"Headphone Amps Compared: Vox Amplug vs. Fender Mustang Micro vs. Boss Waza Air"** — comparison format; affiliate-ready
- **"Is a Silent Guitar Worth It? Yamaha SLG Honest Review"** — high-search product; fair multi-tier framing
- **"Acoustic Guitar at Night: What Actually Works (And What Doesn't)"** — problem-first framing

---

### Guitar Ergonomics & Comfort

- **"The Strap Height Experiment: How Much Does It Change Left-Hand Pain?"** — measurable before/after
- **"Classical Position vs. Folk Position: Which Is Better for Older Players?"** — no clear YouTube answer exists
- **"Guitar Support Systems Compared: Ergoplay, GS Mini Strap, Neck-Up"** — niche but loyal audience
- **"Heavy Guitar, Bad Back: The Real Cost of a Les Paul"** — body weight and shoulder strain; relatable hook
- **"Which Chair? The Playing Position Problem Nobody Talks About"** — simple, useful, shareable
- **"Parlour vs. Dreadnought: Fitting the Guitar to Your Body"** — size-matching as comfort fix

---

### Budget Guitar Fixes: $0–$20

- **"The £0 Setup Check You Can Do in 5 Minutes"** — feeler gauge + hex key only
- **"String Lubricant: £3 Fix for Tuning Problems"** — Nut Sauce vs. Big Bends vs. graphite pencil
- **"The Feeler Gauge Setup Kit Under £10"** — gear video; affiliate entry point

---

### Budget Guitar Rescue (After Video 3)

- **"Before You Buy a Pawn Shop Guitar: The 5-Minute Inspection"** — pre-purchase checklist
- **"Is This Guitar Worth Fixing? How to Triage a Cheap Instrument"** — decision framework
- **"What Can't Be Fixed (And How to Spot It)"** — warped necks, cracked top braces, bad fret jobs
- **"The £80 Guitar That Beats My £400 One"** — controversial title, measurable claim, high CTR potential
- **"Student Guitar vs. Beginner Guitar: What's the Actual Difference?"** — terminology explainer with teeth

---

### Practice Systems

- **"The 15-Minute Sprint: How to Practice When You Have No Time"** — positions the forthcoming PDF product
- **"Why 30 Unfocused Minutes Is Worse Than 10 Focused Ones"** — reframes the practice guilt common to this audience
- **"Practice Logging: What to Track (And What to Ignore)"** — PM system applied to practice
- **"How to Keep Playing When Life Gets Busy"** — maintenance mode; high emotional resonance with 35–65 audience
- **"The Warmup Problem: What to Do Before You Play (Especially If Your Hands Hurt)"**

---

### The PM Player

- **"The Parking Lot List: Stop Trying to Fix Everything at Once"** — core PM concept mapped to guitar; useful and unusual
- **"Milestone Thinking vs. Metric Thinking in Guitar Practice"** — explains why subscriber count is the wrong goal
- **"What Sprint Retrospectives Can Teach You About Practice"** — niche angle; highly shareable in PM communities
- **"Why I Stopped Saying 'I'll Practice More Tomorrow'"** — relatable hook into the PM system

---

## Content Gaps and Underexplored Angles

1. **Carpal tunnel as a distinct condition** — forums are full of this question; no channel addresses it clearly and directly. High-priority gap.
2. **Seasonal/humidity content** — guitar setups drift in winter. Repeatable, searchable, and builds returning viewer habit.
3. **The luthier vs. DIY decision** — high emotional charge for this audience (cost anxiety + trust issues); the channel is perfectly positioned to answer it neutrally.
4. **"Viewer rescue" format** — invite a subscriber to send their guitar, document the full fix on camera. Builds community, generates social proof, creates shareable content.
5. **Playing during a flare-up** — most adaptive playing content is setup-focused; no one covers what to do on a bad hand day.
6. **Guitar size and body fit** — short-scale, parlour, and 3/4 guitars are covered in gear content but almost never framed as a comfort/ergonomics fix. Strong channel fit.
7. **One-handed and partial-limb playing** — Django is already in the channel DNA. An episode here would be the most shareable PTI video and draw coverage beyond the guitar niche.

---

## High-Potential Titles, Hooks & Thumbnail Concepts

### 1. "Guitar Action Too High? Fix It for £0 (Most Players Never Try This)"
- **Hook:** "Your guitar is probably fighting you. Here's how to know in 30 seconds."
- **Thumbnail:** Split before/after action measurement; price callout: £0

### 2. "What Nobody Tells You About Playing Guitar With Arthritis"
- **Hook:** "The problem isn't your hands. It's your setup."
- **Thumbnail:** Hands on guitar neck; amber "PLAYING THROUGH IT" series bar

### 3. "Carpal Tunnel and Guitar: What Actually Helps"
- **Hook:** "Your doctor said stop. Here's the smarter middle path."
- **Thumbnail:** Wrist brace + guitar; PTI series card with episode number

### 4. "I Played This £65 Guitar for 30 Days (Here's What I Changed)"
- **Hook:** "Budget guitars aren't bad. They're just unfinished."
- **Thumbnail:** Cheap guitar with price sticker; before/after measurement overlay

### 5. "Why Barre Chords Hurt (It's Probably Not Your Technique)"
- **Hook:** "98% of barre chord pain is a setup problem."
- **Thumbnail:** Close-up barre chord hand + red X on nut slot

### 6. "The Silent Practice Rig That Actually Works (£0 to £150)"
- **Hook:** "Your neighbours will never know. Your playing will still improve."
- **Thumbnail:** Fix Ladder price callouts on dark background; headphones prominent

### 7. "The Guitar I Wish I'd Bought at 50"
- **Hook:** "Scale length matters more than brand name. Here's the proof."
- **Thumbnail:** Two guitars side by side; measurement callout on scale length

### 8. "Guitar Setup: Pay a Luthier or Do It Yourself?"
- **Hook:** "£75–£100 for something you can learn in an afternoon."
- **Thumbnail:** Luthier invoice vs. £10 feeler gauge kit; before/after action

### 9. "What Happens to Your Guitar in Winter"
- **Hook:** "The neck that was perfect in August is fighting you by December."
- **Thumbnail:** Guitar with humidity/temperature overlay; seasonal before/after

### 10. "The £0 Silent Practice Trick"
- **Hook:** "No gear. No amp. No complaints from next door. Here's how."
- **Thumbnail:** Foam mute on guitar strings; £0 callout; clock showing 11pm

---

## Format Notes

- **Comparison videos** (headphone amps, string gauges, guitar sizes) are highly shareable in this niche and affiliate-ready for Phase 2 monetisation
- **Seasonal hooks** (winter humidity, summer travel) create a recurring content calendar anchor
- **"Viewer rescue" episodes** should be considered once the channel has 50–100 comments per video — strong community builder
- **Shorts / Reels cut-downs:** The before/after measurement moments in every video are natural 30-second Reels clips — no face required, consistent with the channel's existing identity rules

---
---

# Update — September 2026

**Cross-checked against:** Pre-Launch Guide, Channel Ops Guide, Arthritis script draft, and the Final_v7 video order spreadsheet (Videos 1–8 already planned/scripted: $10 Setup, Arthritis, Pawn Shop Rescue, Barre Chords, $4,000 Mistake, Light Strings, Ergonomics for Older Players, PM Identity video). Everything below is additive — it does not restate the June 2026 research above.

## What's Changed Since June

- **YouTube's 2026 algorithm now weights post-watch satisfaction and micro-niche clustering over raw watch time or breadth.** A tight, focused video with a clear payoff outperforms a padded one, and the algorithm is actively rewarding creators who stay narrow rather than broaden their topic range. This is a structural tailwind for this channel specifically — the one-friction-one-fix format is exactly the "tight payoff" shape the algorithm now favours. **Implication:** don't dilute the format to chase breadth; keep runtimes tight and the friction singular, per-video.
- **AI-assisted practice apps (Yousician, Gibson App, Fretello) are a genuine 2026 trend** — real-time pitch/technique feedback via phone mic, adaptive difficulty. None of the marketing around them addresses players whose hand shapes or fingerings don't match the "correct technique" the app is scoring against. That's a clean, unclaimed angle for this channel.
- **Ergonomic seating for guitar ("best guitar chair 2026"-style content) is showing up as its own trending cluster.** Distinct from strap/posture content already in the backlog — this is about the chair/stool itself, which nobody in this niche treats as a Fix Ladder item.
- Carpal tunnel remains confirmed underserved on YouTube specifically (per June note) — search turned up clinical/forum sources and one 2019 surgery vlog, nothing current in the channel's format. Still the single highest-priority gap.

## New Video Ideas

| Working Title | Friction | Fix Ladder Sketch | Audience Appeal |
|---|---|---|---|
| **"I Ran a Guitar App's 'Correct Technique' Score Against My Own Hands"** | AI practice apps grade fingering against one standard hand shape; players with arthritis, limited reach, or asymmetric mobility get marked "wrong" for playing the only way that works for them | Free: ignore the app's technique score, keep its rhythm/pitch feedback only · £0–5: reposition phone mic/angle so the app reads note accuracy, not hand form · £25+: pair app practice with a partial capo so fingerings match what the app expects · Premium: none needed — this is a mindset fix, not a gear one | High — rides the AI-app trend wave while making the channel's core point (there's no one correct way to play) concrete and current |
| **"Does a Better Chair Fix More Than a Better Guitar?"** | Wrist and shoulder strain often traced to guitar setup is sometimes actually seating height/angle | Free: adjust existing chair height so forearm sits level with the bridge · £15–20: footstool or guitar wedge · £30–80: dedicated ergonomic guitar stool · £100+: adjustable-height stool with back support | Medium-high — taps the emerging "guitar chair" search cluster nobody in this niche has claimed yet |
| **Carpal Tunnel and the Guitar (companion to the planned Arthritis video)** | Distinct compression mechanism from arthritis; most existing content is generic hand-health advice, not guitar-specific | Free: wrist angle check + neck relief (same free check as arthritis video, different why) · £5–15: lighter strings · £25–60: professional setup, same as arthritis tier · £80+: partial capo / shorter scale | High — still the single most-requested, least-served "Playing Through It" topic per the June research; natural next episode after Arthritis (V2) |
| **"Recovering From Hand Surgery: Getting Back to Guitar"** | No content addresses the specific return-to-playing period after carpal tunnel release, trigger finger surgery, or joint replacement | Free: reduced-tension tuning while healing · £5–15: extra-light strings for the comeback period · £25–60: setup revisit once grip strength changes · £80+: capo system to reduce reach during recovery | Medium — smaller search volume but very high loyalty/shareability; natural extension of Playing Through It once Carpal Tunnel episode exists |
| **"Short Scale Isn't a Beginner Guitar — Here's Who Actually Plays One"** | Stigma that short-scale = starter instrument discourages adult players from switching even when it would remove real friction | Free: measure your own reach/stretch on current guitar vs. a short-scale to see the actual difference · £5–20: n/a (this is a decision video, not a parts video) · £150–300: budget short-scale (Squier Mini tier) · £300+: quality short-scale electric/acoustic | Medium-high — counters a real objection the channel's own audience likely has; pairs with the existing "Guitar I Wish I'd Bought at 50" idea from June but reframes it as credibility-building rather than nostalgia |

## Content Gaps Confirmed Still Open

1. **Carpal tunnel** — reconfirmed via fresh search; still the top gap, now with a natural surgery/recovery follow-on episode identified above.
2. **AI practice-app compatibility with non-standard hands** — new gap, didn't exist as a searchable topic in June; time-sensitive while the AI-app trend is fresh.
3. **Seating/chair ergonomics as a distinct Fix Ladder item** — previously folded into general "ergonomics," but the search trend suggests it deserves its own video rather than a mention inside the strap-height video.

**Creator note (Sept 2026):** Shorts are off the table — not a format this channel is pursuing. Don't propose a Shorts series or Shorts cadence in future research passes.

## Titles, Hooks, Thumbnails for the Highest-Potential New Ideas

**"I Ran a Guitar App's 'Correct Technique' Score Against My Own Hands"**
- Hook: "The app said my fingering was wrong. My hands disagreed."
- Thumbnail: phone showing app's technique score overlaid on a close-up of a hand playing a non-standard chord shape; no text needed beyond a score number

**Carpal Tunnel and the Guitar**
- Hook: "Different condition, different mechanism — same honest answer: it's mostly the setup."
- Thumbnail: matches existing "Playing Through It" template (navy bottom bar, amber series label, hands on neck) for series consistency

**"Does a Better Chair Fix More Than a Better Guitar?"**
- Hook: "You changed the strings. You changed the action. Did you change the chair?"
- Thumbnail: Type 3 template (product + result) — stool on the left, relaxed shoulder/wrist angle on the right

## Note on Format

All ideas above fit within the existing Fix Ladder / cold-open / play-out structure with no departures from house style. Nothing here requires new visual identity work — the AI-app and chair videos slot into the existing Type 2/Type 3 thumbnail templates, and Carpal Tunnel slots directly into the Type 4 "Playing Through It" template already defined in the Pre-Launch Guide.
